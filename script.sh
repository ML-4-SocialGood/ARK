...
SPECIES_LIST=("Stoat") # <--- 替换为你实际的物种列表

DATA_PROTOCOL="p4"          # 数据集所在的真实协议文件夹 (不变)
RUN_PROTOCOL="p4_no_meta"   # 运行时的独立协议名 (不带元数据的消融测试)
MODEL="gpt-5.4"
LIMIT=50
...
for SPECIES in "${SPECIES_LIST[@]}"; do
    ...
    # 自动寻找 JSON 标注文件 (严格匹配 N4 格式，并兼容多层级嵌套目录)
    ANNOTATION_FILE=$(find annotations -type f -path "*/${SPECIES}/${DATA_PROTOCOL}/${SPECIES}_CIR_P4_N4.json" 2>/dev/null | head -n 1)
