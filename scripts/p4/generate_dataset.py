...
def main():
    parser = argparse.ArgumentParser(
        description="Generate MCQ Dataset for Animal Re-ID (Protocol 4: Multi-Image Query)"
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    print(f"Initializing P4 Sampler for {args.dataset_name}...")
    ...
    for k in sorted(tasks_by_k.keys()):
        tasks = tasks_by_k[k]
        output_filename = f"{args.dataset_name}_MCQ_P4_N{args.gallery_size}_K{k}.json"
