...
def verify_batch(batch_id: int, tasks: Dict[int, Any], data_root: str) -> List[str]:
    ...
    for k in sorted_ks:
        ...
        if task["query"]["ground_truth_id"] != ref_gt_id:
            errors.append(f"[K={k}] Ground Truth ID mismatch. Expected {ref_gt_id}, got {task['query']['ground_truth_id']}")
        ...
        if curr_gallery_str != ref_gallery_str:
            errors.append(f"[K={k}] Gallery does not match the reference gallery (from K={ref_k}). Gallery must be fixed within a batch.")

        # 3. Verify Answer Validity
        # Find the option in gallery that matches GT ID
        correct_opts = [opt["option"] for opt in task["gallery"] if opt["id"] == ref_gt_id]
        if len(correct_opts) != 1:
            errors.append(f"[K={k}] Gallery must contain exactly one correct option. Found {len(correct_opts)}.")
        elif task["answer"] != correct_opts[0]:
            errors.append(f"[K={k}] Answer field '{task['answer']}' does not match the correct option '{correct_opts[0]}'.")

        # 4. Verify Query Set Growth (Incremental)
        curr_query_images = set(task["query"]["image_paths"])
        if len(curr_query_images) != k:
            errors.append(f"[K={k}] Query image count mismatch. Expected {k}, got {len(curr_query_images)}.")
        
        if k > ref_k:
            # Current query set must be a superset of the previous one
            if not prev_query_images.issubset(curr_query_images):
                errors.append(f"[K={k}] Query set is not a superset of K={k-1}. Incremental growth violation.")
        ...
        pos_img_in_gallery = next((opt["image_path"] for opt in task["gallery"] if opt["id"] == ref_gt_id), None)
        if pos_img_in_gallery and pos_img_in_gallery in curr_query_images:
            errors.append(f"[K={k}] DATA LEAKAGE: Positive gallery image '{pos_img_in_gallery}' is present in the query set.")
        ...
        for opt in task["gallery"]:
            if not check_image_exists(opt["image_path"], data_root):
                errors.append(f"[K={k}] Gallery image not found on disk: {opt['image_path']}")
    ...
    for f in os.listdir(p4_dir):
        if f.endswith(".json") and "_P4_" in f:
            ...
            try:
                ...
                n_part = next((p for p in parts if p.startswith("N") and p[1:].isdigit()), None)
                if n_part:
                    n = int(n_part[1:])
...
def main():
    ...
    parser.add_argument("--dataset_name", type=str, default=None, help="Name of the dataset (e.g., BelugaID). If not provided, verifies all datasets.")
    parser.add_argument("--annotations_dir", type=str, default="annotations", help="Root of annotations")
    parser.add_argument("--data_root", type=str, default=".", help="Root for checking image paths (usually project root)")
    ...
    if args.dataset_name:
        ...
        print(f"Found {len(datasets)} datasets in '{args.annotations_dir}'. Verifying all...")
