...
def analyze_file(file_path):
    ...
    # Expected format: {Species}_I2I_P1_N{N}.json
    ...
    # annotations/{DatasetName}/p1/filename
...
def main():
    ...
    all_stats = []
    print(f"Scanning '{args.annotations_dir}' for P1 JSON files...")

    for root, dirs, files in os.walk(args.annotations_dir):
        # We are looking for files inside a 'p1' subdirectory
        if os.path.basename(root) == "p1":
            for file in files:
    ...
    if not all_stats:
        print("No valid P1 annotation files found.")
        return
