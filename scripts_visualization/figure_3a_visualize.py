...
def plot_reid_reasoning_accuracy():
    ...
    # Qwen 家族 - 统一线宽，保持朴素美感
    ax.plot(x_labels, qwen_122b, marker='s', markersize=4.5, color='#08306B', 
            linewidth=1.2, label='Qwen3.5-122B')
    ax.plot(x_labels, qwen_35b, marker='o', markersize=4.5, color='#4F81BD', 
            linewidth=1.2, label='Qwen3.5-35B')
    ax.plot(x_labels, qwen_08b, marker='^', markersize=4.5, color='#9FB6CD', 
            linewidth=1.2, label='Qwen3.5-0.8B')

    # Gemma3 - 使用高级赤土色/深橘色作为对比
    ax.plot(x_labels, gemma3_27b, marker='D', markersize=4.5, color='#A9553B', 
    ...
    ax.legend(loc='center right', frameon=True, edgecolor='#E0E0E0', framealpha=0.9, fontsize=7.5, handlelength=1.2, handletextpad=0.4)
