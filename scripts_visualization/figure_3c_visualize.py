...
def plot_metadata_impact():
    ...
    # 3. 绘制并列分组柱状图
    # 使用高级、柔和的现代学术配色 (Nature/Science 极简风)
    palette = {'NM': '#00718B', 'FO': '#ECA638', 'CR': '#8E9CA3', 'Overall': '#D26466'}
    
    sns.barplot(
        data=df, x='Species', y='Accuracy', hue='Metadata',
        palette=palette, ax=ax, 
        edgecolor='white', linewidth=0.6 # 纯色搭配白色描边，去除斜线，画面更干净通透
    )
    ...
    ax.tick_params(axis='x', which='major', labelsize=6.5)  # 对齐 3d 的 X 轴物种标签字号
    ...
    ax.legend(title=None, loc='upper left', ncol=2, frameon=False, 
              fontsize=6.5, handlelength=1.0, handletextpad=0.3, columnspacing=0.6)
