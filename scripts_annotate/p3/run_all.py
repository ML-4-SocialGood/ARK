...
def main():
    ...
    generate_script = os.path.join(project_root, "scripts_annotate", "p3", "generate_dataset.py")
    analyze_script = os.path.join(project_root, "scripts_annotate", "p3", "analyze_dataset.py")
    ...
    for i, species in enumerate(species_list):
        ...
        for N in gallery_sizes:
            for M in num_positives_list:
                ...
                base_name = f"{species}_MIA_P3_N{N}_M{M}"
                json_file = os.path.join(project_root, "annotations", species, "p3", f"{base_name}.json")
                plot_file = os.path.join(project_root, "annotations", species, "p3", f"{base_name}_distribution.png")
